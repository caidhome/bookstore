...
import lzit.rj.bookstore.utils.PicUtils;
import lzit.rj.bookstore.utils.UploadUtils;
			...
			try {
				...
				for (FileItem item : items) {
					...
					if (item.isFormField()) {
						...
						// 封装imgurl
						String mixPax = "/upload" + uuidDir	+ "/" + uuidname;
						map.put("imgurl", new String[] { mixPax.substring(0, mixPax.lastIndexOf(".")) });
				...
				service.addBook(p);
				// 得定向到首页
				response.sendRedirect(request.getContextPath()+"/findAllBookSys");
				return;
